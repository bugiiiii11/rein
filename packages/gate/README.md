...
- **Surge pricing (profit guard).** A keyless facilitator bills the seller gas plus a margin per settlement, so a cent-level price can lose money on a congested day. With `surge: { cost: settlementCostOracle({ rpcUrl, ethUsdFeed: BASE_ETH_USD_FEED, pricing: { url: PAYAI_PRICING_URL, network: 'eip155:8453' } }) }` every route quotes `max(list, 2 x cost)`, refuses `503 price_ceiling` above `5 x list`, and refuses `503 price_unavailable` when the cost is unknown — never the list price. A payment signed at an earlier, higher quote still settles, at its own value.
