...
- **Payer screening.** Allow/deny lists plus a dynamic `screen.check` hook (reputation plugs in here — see [`@rein/graph`](https://www.npmjs.com/package/@rein/graph)'s `payerCheck`), checked *before* verify/settle, so a blocked payer costs you nothing.
...
- **Pluggable rails.** The same gate runs against the mock facilitator ([`@rein/mock-rails`](https://www.npmjs.com/package/@rein/mock-rails)) or the real hosted facilitator ([`@rein/x402-rails`](https://www.npmjs.com/package/@rein/x402-rails)) — a two-method structural seam.
