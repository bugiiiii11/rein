...
- **Receipts either way.** Every paywall encounter — allowed, denied, settled — becomes a `Receipt`; stream them out with `onReceipt`. `receipts()` keeps only the most recent 1000 (`maxReceipts`) — a session log, not the record — and `onReceipt` sees every one.
- **Settlement reported, so gaps are visible.** When a payment settles, the guard tells the engine (fire-and-forget — a failed report can never affect a payment that already succeeded), and an allowance that never settles shows up in `client.reconciliation()` as a gap rather than disappearing. Set `reportSettlement: false` where an independent indexer reports instead, which is the stronger evidence: a guard reporting its own settlement is the spender vouching for itself.
- **A guarded call is a heartbeat.** If the engine is watching this agent for silence (`client.watchLiveness`), every intent counts as a sighting — a denied one too, because a blocked agent is alive. Only an agent that goes quiet on purpose needs `client.heartbeat(agentId)`; without one, a dead-man alarm would really be a no-spend alarm.
