...
- **The HTTP admin surface is fail-closed by construction.** `buildSignerServer` refuses to build without either an `adminToken` (≥16 chars, compared with a timing-safe digest, sent as `Authorization: Bearer` or `X-Api-Key`) or an explicit `adminAuth: 'off'`. Those routes mint spending authority against custodied wallets, so forgetting a field cannot produce an open signer — only a deliberate statement can.
