...
- **Dead-man monitoring** — declare an expected-activity interval per agent and the engine notices when one stops: every intent counts as a sighting (a *denied* one too — a blocked agent is alive), an explicit heartbeat covers the agent that has nothing to buy, and silence ages from `alive` through `late` to `missing` before it is announced, once, through the same one-way channels approvals use. Watching is declared, never inferred, and an engine that has just restarted says `unknown` rather than alarming about silence it did not witness.
- **HTTP API** — `POST /v1/evaluate`, agent + policy CRUD, decision reads, `/v1/keys` (issue/rotate/revoke), `/v1/approvers`, `/v1/approvals`, `/v1/settlements` + `/v1/reconciliation`, `/v1/liveness` + `/v1/agents/:id/heartbeat`. Zod-validated 400s, not 500s.
