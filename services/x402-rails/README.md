...
```ts
import {
  createX402Payer,     // EIP-3009/EIP-712 signer — plugs into @rein/sdk's guard as its `payer`
...
- **The hosted facilitator, for free.** `FacilitatorClient` speaks the x402.org dialect (`DEFAULT_FACILITATOR_URL`, no API key). The same client powers [`@rein/gate`](https://www.npmjs.com/package/@rein/gate)'s real-rails settlement via `facilitatorClientRails`.
